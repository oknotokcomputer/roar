...
int main() {
  ...
  CTRACE_EVENT_INSTANT_STR("cat1", "start");
  for (int i = 0; i < 3; i++) {
    CTRACE_COUNTER_STR("cat1", "iteration", (int64_t)i);
    CTRACE_EVENT_BEGIN_STR("cat1", "event 1");
    sleep(1000);
    CTRACE_EVENT_BEGIN_STR("cat2", "event 2");

    sleep(10000);

    CTRACE_EVENT_END_STR("cat2");
    sleep(1000);
    CTRACE_EVENT_END_STR("cat1");

    sleep(10000);
  }
  CTRACE_EVENT_INSTANT_STR("cat1", "end");
