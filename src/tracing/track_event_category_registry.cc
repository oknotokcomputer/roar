...
void TrackEventCategoryRegistry::EnableCategoryForInstance(
  ...
  CallInstancesCallbacks(category_index);
}
...
void TrackEventCategoryRegistry::CallInstancesCallbacks(
    size_t category_index) const {
  auto category = GetCategory(category_index);
  if (category->instances_callback) {
    category->instances_callback(
        state_storage_[category_index].load(std::memory_order_relaxed),
        category->client_index);
  }
}
