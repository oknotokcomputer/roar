...
class FlowIdGenerator {
  public:
    static PERFETTO_ALWAYS_INLINE uint64_t generate_unique_flow_id() {
      return ++last_used_flow_id;
    }

  private:
  static std::atomic<uint64_t> last_used_flow_id;
};
inline std::atomic<uint64_t> FlowIdGenerator::last_used_flow_id = 0;
