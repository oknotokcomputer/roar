...
struct PERFETTO_EXPORT_COMPONENT Category {
  ...
  void (*instances_callback)(uint32_t instances, size_t client_index) = nullptr;
  Tags tags = {};
 ...
 private:
  ...
  static constexpr const char* CheckIsValidCategory(const char* n) {
    ...
    // static assert in TrackEventCategoryFinder::ValidateCategories().
...
class PERFETTO_EXPORT_COMPONENT TrackEventCategoryRegistry {
 public:
  constexpr TrackEventCategoryRegistry(size_t max_categories,
                                       size_t category_count,
                                       const Category* init_categories,
                                       Category* all_categories,
                                       std::atomic<uint8_t>* state_storage)
      : max_categories_(max_categories),
        category_count_(category_count),
        init_category_count_(category_count),
        init_categories_(init_categories),
        categories_(all_categories),
  ...
  // Copy init_categories_ into categories_. This operation is idempotent and
  // threadsafe because the source is const and the destination (though not
  // const) is only ever changed when used by the cperfetto shared library, in
  // which case init_categories_ is empty and init_category_count_ is 0.
  void Init() {
    for (uint32_t i = 0; i < init_category_count_; i++) {
      categories_[i] = init_categories_[i];
    }
  }

  // The underlying category array has been modified, and now contains `count`
  // valid categories. This is only used by the cperfetto shared library.
  void SetCount(size_t count) { category_count_ = count; }

  size_t category_count() const { return category_count_; }
  size_t max_categories() const { return max_categories_; }

  // Returns a category based on its index.
  Category* GetCategory(size_t index) const {
  ...
  void DisableCategoryForInstance(size_t category_index,
                                  uint32_t instance_index) const;
  void CallInstancesCallbacks(size_t category_index) const;
  ...
  const size_t max_categories_;
  size_t category_count_;
  size_t init_category_count_;
  const Category* init_categories_;
  Category* categories_;
  std::atomic<uint8_t>* const state_storage_;
};

// Holds all the declared categories for one category namespace. This now
// holds all the former const Category logic of TrackEventCategoryRegistry, and
// is used to validate categories and map category strings to indexes at
// compile time.
class PERFETTO_EXPORT_COMPONENT TrackEventCategoryFinder {
 public:
  constexpr TrackEventCategoryFinder(size_t category_count,
  ...
  // Returns a const category based on its index.
 ...
 private:
  ...
  constexpr size_t FindImpl(const char* name,
    ...
    return is_dynamic ? TrackEventCategoryRegistry::kDynamicCategoryIndex
                      : (index == category_count_)
                            ? TrackEventCategoryRegistry::kInvalidCategoryIndex
  ...
  static constexpr size_t CheckIsValidCategoryIndex(size_t index) {
    ...
    // clang-format off
    return index != TrackEventCategoryRegistry::kInvalidCategoryIndex \
      ? index : [] {
        ...
        return TrackEventCategoryRegistry::kInvalidCategoryIndex;
      }();
    // clang-format on
  }
