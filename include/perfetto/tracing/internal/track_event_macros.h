...
#define PERFETTO_INTERNAL_DECLARE_CATEGORIES(...)                             \
  ...
  constexpr ::perfetto::internal::TrackEventCategoryFinder kCategoryFinder(   \
      kCategoryCount,                                                         \
      &kCategories[0],                                                        \
      &g_category_state_storage[0]);                                          \
  PERFETTO_COMPONENT_EXPORT extern ::perfetto::Category g_all_categories[];   \
  PERFETTO_COMPONENT_EXPORT extern ::perfetto::internal::                     \
      TrackEventCategoryRegistry kCategoryRegistry;                           \
  static_assert(kCategoryFinder.ValidateCategories(),                         \
...
#define PERFETTO_INTERNAL_CATEGORY_STORAGE()                                 \
  ...
  PERFETTO_COMPONENT_EXPORT std::atomic<uint8_t>                             \
      g_category_state_storage[kCategoryCount];                              \
  PERFETTO_COMPONENT_EXPORT ::perfetto::Category                             \
      g_all_categories[kCategoryCount];                                      \
  PERFETTO_COMPONENT_EXPORT ::perfetto::internal::TrackEventCategoryRegistry \
...
#define PERFETTO_GET_CATEGORY_INDEX(category)                       \
  ::PERFETTO_TRACK_EVENT_NAMESPACE::internal::kCategoryFinder.Find( \
      category,                                                     \
